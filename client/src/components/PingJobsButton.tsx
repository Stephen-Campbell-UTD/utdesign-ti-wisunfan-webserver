...
interface PingJobsButtonProps {
  ipAddressInfoArray: IPAddressInfo[];
  pingbursts: Pingburst[];
}

interface PingJobsButtonTheme {
  bgColor: Color;
}
const pingJobsButtonThemeImplementations = new ComponentThemeImplementations<PingJobsButtonTheme>();
const tiPingJobsButtonTheme = {
  bgColor: ColorScheme.getColor('redDark', THEME.TI),
};
pingJobsButtonThemeImplementations.set(THEME.TI, tiPingJobsButtonTheme);
const gruvboxPingJobsButtonTheme = {
  bgColor: ColorScheme.getColor('bg1', THEME.GRUVBOX),
};
pingJobsButtonThemeImplementations.set(THEME.GRUVBOX, gruvboxPingJobsButtonTheme);

export function PingJobsButton(props: PingJobsButtonProps) {
  const [shown, setShown] = useState(false);
  const theme = useContext(ThemeContext);
  const activeClass = shown ? 'active' : '';
  const {bgColor} = pingJobsButtonThemeImplementations.get(theme);
  return (
    <div className={['pingJobs_icon_container', theme, activeClass].join(' ')}>
      <svg
        style={{
          overflow: 'visible',
        }}
      ...
      >
        {shown && (
          <g style={{transformOrigin: 'center'}} transform="scale(1.5)">
            <rect x="0" y="0" width="24" height="24" rx={2} fill={bgColor}></rect>
          </g>
        )}
        <path
          fillRule="evenodd"
      ...
      <AnimatePresence>{shown && <PingJobMonitor {...props} />}</AnimatePresence>
    </div>
  );
}
