...
interface SettingsButtonTheme {
  surfaceStyle: React.CSSProperties;
  inputStyle: React.CSSProperties;
}
const settingsButtonThemeImplementations = new ComponentThemeImplementations<SettingsButtonTheme>();
const tiSettingsButtonTheme = {
  surfaceStyle: {
    backgroundColor: ColorScheme.getColor('bg2', THEME.TI),
    // borderTop: `3px solid ${ColorScheme.getColor("red", theme)}`,
...
settingsButtonThemeImplementations.set(THEME.TI, tiSettingsButtonTheme);
const gruvboxSettingsButtonTheme = {
  surfaceStyle: {
...
settingsButtonThemeImplementations.set(THEME.GRUVBOX, gruvboxSettingsButtonTheme);

export default function SettingsButton(props: SettingsButtonProps) {
  const theme = useContext(ThemeContext);
  const [isHovering, setHoverState] = useState(false);
  const [apiLocation, setAPILocation] = useState(APIService.host);
  APIService.host = apiLocation;
  let {inputStyle, surfaceStyle} = settingsButtonThemeImplementations.get(theme);
  surfaceStyle = {
    ...surfaceStyle,
    width: '19.607vw',
    position: 'absolute',
  ...
  return (
    ...
    >
      <motion.svg
        whileTap={{rotate: 90}}
        onClick={() => setHoverState(!isHovering)}
        style={{
          marginLeft: 10,
          cursor: 'pointer',
        }}
