...
import {average, ComponentThemeImplementations, timestampStringToDate} from '../utils';
...
function ipSeries(pingbursts: Pingburst[], destIP: string, label: string, color: Color): IPSeries {
  let data: Datum[] = [];
  for (let pingburst of pingbursts) {
    const validPingburst = pingburst.records.length !== 0 && pingburst.records[0].destIP === destIP;
    if (!validPingburst) {
      continue;
    }
  ...
  return {data, color, id: destIP, label};
}
...
> {
  ...
  render() {
    ...
    const {pingbursts, ipAddressInfoArray} = this.props;
